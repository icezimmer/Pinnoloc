...
class PositionLoss(torch.nn.Module):
    ...
    def collocation_points(n_collocation, n_features, device, requires_grad=True):
        # Collocation points for the physics loss representing RSSI with normal distribution
        P_collocation = torch.randn(n_collocation, 4, device=device, requires_grad=requires_grad)
        other_collocation = torch.randn(n_collocation, n_features - 4, device=device, requires_grad=False)
        # P_collocation = -3.0 + 6.0 * torch.rand(n_collocation, 1, device=device, requires_grad=requires_grad)
    ...
    def forward(self, model, inputs, targets):
        if self.lambda_rss == 0.0 and self.lambda_azimuth == 0.0 and self.lambda_bc == 0.0:
            ...
            dx_dP = (self.std_target[0:1] / self.std_input[0:4]) * dx_dP
            dy_dP = (self.std_target[1:2] / self.std_input[0:4]) * dy_dP
            # da_dP = (self.std_input[4:8] / self.std_input[0:4]) * da_dP
            ...
            x_collocation = x_collocation - model.anchor_x  # (N, 4)
            y_collocation = y_collocation - model.anchor_y  # (N, 4)
            distance_2 = torch.clamp(torch.pow(x_collocation, 2) + torch.pow(y_collocation, 2), min=1e-8)  # (N, 4)
            ...
            azimuth_loss = 0.0
            for anchor_id in range(4):
                da_dP = list_da_dP[anchor_id]
                da_dP = (self.std_input[4+anchor_id:5+anchor_id] / self.std_input[0:4]) * da_dP
