...
#             target = target.to(self.device)
#             output = self.model(input_)
#             loss = self.criterion(output, target, physics)
#             loss.backward()
#             self.optimizer.step()
...
#                 target = target.to(self.device)
#                 output = self.model(input_)
#                 running_loss += self.criterion(output, target, physics).item()
...
class TrainPhysicsModel:
    ...
    def _epoch(self, dataloader):
        ...
        for item in tqdm(dataloader):
            ...
            input_ = input_.to(self.device)
            target = target.to(self.device)
            loss = self.criterion(self.model, input_, target, physics)
            loss.backward()
            self.optimizer.step()
